...
<body <?php if(isset($_GET['print'])){ ?> onload="window.print();window.close();"  <?php }  ?>>
<?php
$customer = $this->settings->getCustomer($invoice['customer']);
$reservation = $this->stock->getSale($invoice['invoice_id']);
$items = json_decode($invoice['invoice_item'],true);
?>
<div class="container" >
    <div class="row">
        <div class="col-xs-12">
		...
		<div class="row">
		<div class="col-xs-6">
		<img src="<?php echo $this->settings->getSettings()['slogo'] ?>" alt="" width="120">
		<address>
		...
		<div class="btn-group pull-right" role="group">
         <a href="<?php echo base_url('dashboard/invoices') ?>" class="btn btn-primary"><i class="fa fa-arrow-long-left"></i> Back
          </button>
		  <?php  if($invoice['payment_status']=="0"){ ?>
          <a onclick="return warning(this);" href="<?php echo base_url('dashboard/invoice/'.$invoice['invoice_id'].'?markAspaid=true') ?>" class="btn btn-warning">Mark As Paid
          </a>
		  <?php } ?>
          <a onclick="window.open(this.href,'width=400;hieght=400;','400','400'); return false" href="<?php echo base_url('dashboard/invoice/'.$invoice['invoice_id'].'?print=true') ?>" class="btn btn-primary">Print Big Invoice
          </a>
		  <a onclick="window.open(this.href,'width=400;hieght=400;','400','400'); return false" href="<?php echo base_url('dashboard/print_recipt/'.$reservation['reciept_id']) ?>" class="btn btn-primary">Print Small Invoice 
          </a>
          </div>
		<?php } ?> 
		</div>
			...
			<h2>Invoice Reciept</h2><h3 class="pull-right"> # <?php echo $invoice['invoice_id'] ?></h3>
    		</div>
    		<hr>
    		<div class="row">
    			<div class="col-xs-6">
